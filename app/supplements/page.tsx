...
export const metadata: Metadata = {
    title: 'Luxury Supplements | DISSUN',
    description: 'Complete your look with our collection of luxury health supplements and fine items.',
}

async function getSupplements() {
    const supplements = await db.products.findMany({
        where: {
            published: true,
            category: 'ACCESSORY'
        },
...
export default async function SupplementsPage() {
    const supplements = await getSupplements() as any[]
        ...
        <div className="bg-metamask-peach min-h-screen">
            ...
            <section className="max-w-7xl mx-auto px-6 lg:px-8 py-24 relative overflow-hidden">
                ...
                <div className="text-center relative animate-fade-in">
                    <div className="inline-flex items-center gap-3 px-6 py-2 bg-metamask-orange/10 rounded-full mb-8">
                        <Settings className="w-4 h-4 text-metamask-orange" />
                        <span className="text-metamask-orange font-black uppercase tracking-[0.2em] text-xs">Essential Details</span>
                    </div>

                    <h1 className="metamask-heading text-6xl lg:text-9xl mb-8">
                        Luxury <span className="text-metamask-orange">Supplements</span>
                    </h1>
                    ...
                    <p className="text-xl lg:text-2xl font-bold text-metamask-black/80 max-w-4xl mx-auto leading-relaxed italic border-x-4 border-metamask-orange/20 px-8">
                        "The art of the collection lies in the details. Protect, display, and enhance your treasures with our curated selection of fine supplements."
                    </p>
                </div>
            </section>
                    ...
                    {[
                        {
                            ...
                            desc: 'Hand-stitched leathers, solid hardwoods, and precision-engineered tools for the serious collector.',
                            icon: Settings
                        },
                        {
                            title: 'Ultimate Protection',
                            desc: 'Ensure your collection remains in showroom condition with our professional care solutions.',
                            icon: Shield
                        },
                        {
                            title: 'Functional Design',
                            desc: 'Elegance meets utility in every accessory, designed to complement your luxury lifestyle.',
                            icon: Zap
                        }
                    ].map((item, i) => (
            ...
            <section className="max-w-7xl mx-auto px-6 lg:px-8 pb-32">
                <div className="mb-12 flex justify-between items-end">
                    <div>
                        <h2 className="text-4xl font-black text-metamask-black">Essential Detail</h2>
                        <p className="text-metamask-black/40 font-bold mt-2 italic">Refine your standard</p>
                    </div>
                    <div className="text-metamask-black/20 font-black text-6xl">{supplements.length}</div>
                </div>
                ...
                {supplements.length > 0 ? (
                    ...
                    <div className="py-32 text-center bg-white/20 backdrop-blur-sm rounded-[4rem] border border-dashed border-black/10">
                        <p className="text-2xl font-bold text-metamask-black/30 italic">
                            Our accessory collection is currently being curated.<br />
                            Please visit us again soon for new arrivals.
                        </p>
                    </div>
                )}
            </section>
